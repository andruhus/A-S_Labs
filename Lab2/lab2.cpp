...
int main()
{
    RedBlackTree<Department> A;
    int  n;
    cout << blue << "Enter number departments" << endl;
    cin >> n;
    cin.ignore();
    cout << endl;
    string name;
    for (int i = 0; i < n; i++)
    {
        cout << i + 1 << "-th department name:" << endl;
        getline(cin, name);
        cout << i + 1 << "-th department workers:" << endl;
        string models;
        getline(cin, models);
        A.Insert(Department(name, Parse(models)));
    }
    A.Print();
    return 0;
}